...
export class AdminService {
  ...
  async listUsers(pagination: PaginationDto) {
    ...
    if (pagination.cursor)
      qb.andWhere("u.id < :cursor", { cursor: pagination.cursor });
  ...
  async listBusinesses(pagination: PaginationDto) {
    ...
    if (pagination.cursor)
      qb.andWhere("b.id < :cursor", { cursor: pagination.cursor });
    const items = await qb.getMany();
    const hasMore = items.length > limit;
    if (hasMore) items.pop();
    return {
  ...
  async createPayout(dto: CreatePayoutDto, _idempotencyKey?: string) {
    const provider = await this.providerRepo.findOne({
