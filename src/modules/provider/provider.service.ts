...
export class ProviderService {
  ...
  private async assertAccess(user: RequestContext, providerId: string, allowedRoles?: string[]): Promise<void> {
    const membership = user.memberships.find((m) => m.providerId === providerId);
  ...
  async create(userId: string, dto: CreateProviderDto): Promise<{ provider_id: string; status: string; created_at: Date }> {
    ...
    return { provider_id: provider.id, status: provider.status, created_at: provider.created_at };
  }
  ...
  async findOne(user: RequestContext, id: string) {
    ...
    const provider = await this.providerRepo.findOne({
      where: { id },
      select: ['id', 'legal_name', 'trading_name', 'provider_type', 'status', 'default_currency', 'created_at'],
    });
  ...
  async update(user: RequestContext, id: string, dto: UpdateProviderDto) {
    ...
    if (dto.description !== undefined) provider.description = dto.description ?? null;
    if (dto.default_currency != null) provider.default_currency = dto.default_currency;
    await this.providerRepo.save(provider);
    return { provider_id: provider.id, updated_at: provider.updated_at };
  }

  async addProduct(user: RequestContext, providerId: string, dto: CreateProductDto) {
    await this.assertAccess(user, providerId, PROVIDER_ROLES);
    const provider = await this.providerRepo.findOne({ where: { id: providerId } });
    if (!provider) throw new NotFoundException('Provider not found');
    const existing = await this.productRepo.findOne({ where: { provider_id: providerId, sku: dto.sku } });
  ...
  async findProducts(user: RequestContext, providerId: string, pagination: PaginationDto) {
    await this.assertAccess(user, providerId, PROVIDER_ROLES);
    ...
    const qb = this.productRepo
      ...
      .orderBy('p.id', 'DESC')
      .take(limit + 1)
      .select(['p.id', 'p.sku', 'p.name', 'p.category', 'p.unit', 'p.price', 'p.currency', 'p.is_active', 'p.allowed_sizes', 'p.image_urls']);
  ...
  async updateProduct(user: RequestContext, providerId: string, productId: string, dto: UpdateProductDto) {
    await this.assertAccess(user, providerId, PROVIDER_ROLES);
    const product = await this.productRepo.findOne({ where: { id: productId, provider_id: providerId } });
    ...
    if (dto.allowed_sizes !== undefined) product.allowed_sizes = dto.allowed_sizes?.length ? dto.allowed_sizes : null;
    ...
    if (dto.image_urls !== undefined) product.image_urls = dto.image_urls?.length ? dto.image_urls.slice(0, 3) : null;
    await this.productRepo.save(product);
    return { product_id: product.id, updated_at: product.updated_at };
  }

  async deleteProduct(user: RequestContext, providerId: string, productId: string): Promise<void> {
  ...
  async addLocation(user: RequestContext, providerId: string, dto: CreateProviderLocationDto) {
    await this.assertAccess(user, providerId, EDIT_ROLES);
    const provider = await this.providerRepo.findOne({ where: { id: providerId } });
    if (!provider) throw new NotFoundException('Provider not found');
    const location = this.locationRepo.create({
