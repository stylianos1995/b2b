...
export class DiscoveryService {
  ...
  async findProviders(
    user: RequestContext,
    filters: { provider_type?: string; postcode?: string; radius_km?: number; category?: string },
    ...
    if (filters.provider_type) {
      qb.andWhere('p.provider_type = :providerType', { providerType: filters.provider_type });
    }
    if (filters.category) {
      qb.andWhere('EXISTS (SELECT 1 FROM products pr WHERE pr.provider_id = p.id AND pr.category = :category AND pr.is_active = true)', {
  ...
  async getProviderPublic(user: RequestContext, providerId: string) {
    ...
    const provider = await this.providerRepo.findOne({
      ...
      select: ['id', 'trading_name', 'provider_type', 'description', 'min_order_value', 'lead_time_hours'],
    });
  ...
  async getProviderProducts(user: RequestContext, providerId: string, pagination: PaginationDto) {
    this.assertBuyer(user);
    const provider = await this.providerRepo.findOne({ where: { id: providerId, status: 'active' } });
    ...
    const qb = this.productRepo
      ...
      .orderBy('p.id', 'ASC')
      .take(limit + 1)
      .select(['p.id', 'p.name', 'p.sku', 'p.category', 'p.unit', 'p.unit_size', 'p.price', 'p.currency', 'p.allowed_sizes', 'p.image_urls']);
    if (pagination.cursor) {
      qb.andWhere('p.id > :cursor', { cursor: pagination.cursor });
    }
    const items = await qb.getMany();
