  ...
  async getPdf(
    ...
    const PDF =
      typeof PDFDocument === "function"
        ? PDFDocument
        : (PDFDocument as { default?: typeof PDFDocument }).default;
    ...
    return new Promise((resolve, reject) => {
      ...
      for (const line of lines) {
        ...
        doc.text(`${unitPrice.toFixed(2)} ${currency}`, 340, rowY, {
          width: 80,
        });
        doc.text(`${lineTotal.toFixed(2)} ${currency}`, 430, rowY, {
          width: 90,
        });
      ...
      doc.text(`${subtotal.toFixed(2)} ${currency}`, 430, summaryY, {
        width: 90,
      });
      doc.text(`Tax:`, summaryX, summaryY + 18, { width: 100 });
      doc.text(`${taxTotal.toFixed(2)} ${currency}`, 430, summaryY + 18, {
        width: 90,
      });
      doc.font("Helvetica-Bold");
      doc.text(`Total:`, summaryX, summaryY + 36, { width: 100 });
      doc.text(`${total.toFixed(2)} ${currency}`, 430, summaryY + 36, {
        width: 90,
      });
