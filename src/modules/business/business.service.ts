...
export class BusinessService {
  ...
  private async assertAccess(user: RequestContext, businessId: string, allowedRoles?: string[]): Promise<void> {
  ...
  async create(userId: string, dto: CreateBusinessDto): Promise<{ business_id: string; created_at: Date }> {
  ...
  async findOne(user: RequestContext, id: string) {
    ...
    const business = await this.businessRepo.findOne({
      where: { id },
      select: ['id', 'legal_name', 'trading_name', 'business_type', 'status', 'default_currency', 'default_delivery_address_id', 'created_at'],
    });
  ...
  async update(user: RequestContext, id: string, dto: UpdateBusinessDto) {
    ...
    if (dto.default_currency != null) business.default_currency = dto.default_currency;
    await this.businessRepo.save(business);
    return { business_id: business.id, updated_at: business.updated_at };
  }

  async addLocation(user: RequestContext, businessId: string, dto: CreateLocationDto) {
  ...
  async findLocations(user: RequestContext, businessId: string) {
    ...
    const locations = await this.locationRepo.find({
      ...
      select: ['id', 'address_line_1', 'city', 'postal_code', 'country', 'is_default'],
