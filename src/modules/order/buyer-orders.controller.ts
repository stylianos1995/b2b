...
export class BuyerOrdersController {
  ...
  async create(
    ...
    return this.orderService.placeOrder(user, this.businessId(req), dto, idempotencyKey);
  }
  ...
  async list(
    ...
    return this.orderService.listByBusiness(user, this.businessId(req), status, pagination ?? {});
  }
  ...
  async cancel(
    ...
    return this.orderService.cancelByBusiness(user, this.businessId(req), id, dto?.reason);
  }
}
