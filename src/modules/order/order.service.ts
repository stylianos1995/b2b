...
export class OrderService {
  ...
  async placeOrder(
    ...
    if (!location)
      throw new BadRequestException(
        "Delivery location not found or not owned by your business",
      );
    ...
    if (products.length !== productIds.length)
      throw new BadRequestException(
        "One or more products not found or not from this provider",
      );
    ...
    for (const line of dto.lines) {
      ...
      if (allowedSizes.length > 0) {
        ...
        if (sizeInBase == null) {
          throw new BadRequestException(
            `Invalid size "${line.unit}" for product "${p.name}".`,
          );
        }
    ...
    const taxTotal = lines.reduce(
      (sum, l) => sum + (l.line_total - l.quantity * l.unit_price),
      0,
    );
  ...
  async listByBusiness(
    ...
    if (pagination.cursor)
      qb.andWhere("o.id < :cursor", { cursor: pagination.cursor });
  ...
  async listByProvider(
    ...
    if (filters?.date_from)
      qb.andWhere("o.created_at >= :dateFrom", {
        dateFrom: `${filters.date_from}T00:00:00.000Z`,
      });
    if (filters?.date_to) {
  ...
  async updateStatus(
    ...
    if (status === "preparing") {
      ...
      await this.orderRepo.save(order);
      this.orderEventsProducer.orderPrepared({
        order_id: order.id,
        provider_id: providerId,
      });
    } else {
      if (!["confirmed", "preparing"].includes(order.status))
        throw new BadRequestException("Order cannot be marked shipped");
