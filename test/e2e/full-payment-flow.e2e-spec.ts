...
describe('Full payment flow (e2e)', () => {
  ...
  async function truncateAll(ds: DataSource) {
    ...
    await qr.connect();
    try {
      await qr.query('TRUNCATE TABLE payments, invoice_lines, invoices, order_lines, deliveries, ratings, orders, products, business_users, provider_users, locations, businesses, providers, sessions, users RESTART IDENTITY CASCADE');
    } finally {
      await qr.release();
    }
  }
  ...
  it('runs full lifecycle: register → business → provider → product → order → confirm → deliver → invoice → checkout → webhook → assert idempotency', async () => {
    ...
    const getBiz = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${buyerToken}`)
      .expect(200);
    deliveryLocationId = (getBiz.body as { default_delivery_address_id: string }).default_delivery_address_id;
    if (!deliveryLocationId) throw new Error('Business has no default_delivery_address_id');
    ...
    await dataSource.query('UPDATE providers SET status = $1 WHERE id = $2', ['active', providerId]);
    ...
    const amountTotal = currency === 'jpy' || currency === 'krw' ? Math.round(total) : Math.round(total * 100);
  ...
  it('rejects webhook with invalid signature', async () => {
    const payload = JSON.stringify({ type: 'checkout.session.completed', data: { object: { id: 'cs_fake' } } });
    await request(app.getHttpServer())
      .post(`${PREFIX}/payments/webhook`)
